...
class GaussianPsfResult:
    ...
    Parameters follow the calibration/export convention used by this package's
    integrated PSF workflow:
      sigma_x, sigma_y are the axis widths reconstructed from the integrated
      PSF fit parameterization used by the reference calibration code,
      ellipticity = sigma_x / sigma_y

    These widths are calibration-model widths, not guaranteed to match the
    axis sigmas from an independently fitted plain exponential Gaussian.
    """
    x: float
    y: float
    sigma_x: float
...
def fit_gaussian_psf(patch: Image2D, theta: float | None = None) -> GaussianPsfResult | None:
    ...
    This uses the package's integrated 7-parameter PSF model so calibration
    and runtime use a consistent ellipticity definition.
    """
